...
def results_to_file(representation, data, xi, ix, resultsfile):
    sims = prepare_similarities(representation, ix)
    correct_add = 0.0
    correct_mul = 0.0
    with open(resultsfile, 'w') as outfile:
        for a, a_, b, b_ in data:
            ...
            if b_mul == b_:
                correct_mul += 1
            outfile.write(b_+'\t'+b_add+'\t'+b_mul+'\n')

        accuracy_cosadd = correct_add/len(data)
        accuracy_cosmul = correct_mul/len(data)
        return accuracy_cosadd, accuracy_cosmul
