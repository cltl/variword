...
def combinations_to_file_ws(model_list, outfile):
    ...
    for combination in combinations(model_list, 2):
        ...
        outfile.write('python hyperwords/ws_eval_models.py $wc results/$UUID/ws/'+model1+' results/$UUID/ws/'+model2+' $RESULTSDIR/ws/'+name+'.tsv $UUIDRES\n')
...
def combinations_to_file_analogy(model_list, outfile):
    ...
    for combination in combinations(model_list, 2):
        ...
        outfile.write('python hyperwords/analogy_eval_models.py $wc results/$UUID/analogies/'+model1+' results/$UUID/analogies/'+model2+' $RESULTSDIR/analogies/'+name+'.tsv $UUIDRES\n')
