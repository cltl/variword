...
char wvocab_file[MAX_STRING], cvocab_file[MAX_STRING], pinit_file[MAX_STRING];
...
void ReadParamInit(struct vocabulary *wv, struct vocabulary *cv, char *pinit_file) {
   ...
   a = posix_memalign((void **)&syn1neg, 128, (long long)cv->vocab_size * layer1_size * sizeof(real));
    ...
    for (a = 0; a < wv->vocab_size; a++) {
        for (b = 0; b < layer1_size; b++) fread(&syn0[a * layer1_size + b], sizeof(real), 1, f);
    }
    for (a = 0; a < cv->vocab_size; a++) {
...
int main(int argc, char **argv) {
  ...
  if ((i = ArgPos((char *)"-pinit", argc, argv)) > 0) strcpy(pinit_file, argv[i + 1]);
